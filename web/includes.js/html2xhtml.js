		...
		switch(state)
			...
			case(0):
				...
				if(Html2Xhtml.charEntities[cc])
					xhtml = xhtml + '&' + Html2Xhtml.charEntities[cc] + ';';
				else
					xhtml = xhtml + c;
			...
			case(2):
				if(Html2Xhtml.isSpaceChar[c])
					{
					xhtml = xhtml + (!insidepre && tagtype > 0 && Html2Xhtml.hasNLBefore[tagname] && xhtml.length && xhtml.substr(xhtml.length - 1, 1) != '\n' ? '\n' : '') + (tagtype > 0 ? '<' : '</') + tagname;
			...
			case(3):
				...
				break;
			case(4):
				xhtml = xhtml + (Html2Xhtml.isEmptyTag[tagname] ? ' />' : '>') + (!insidepre && tagtype < 0 && Html2Xhtml.hasNLAfter[tagname] && p < len && data.substr(p, 1) != '\n' ? '\n' : '');
			...
			case(5):
				...
				if(c == '"' || c == "'")
					...
					attrname = attrname + c.toLowerCase();
			...
			case(6):
				if(Html2Xhtml.isSpaceChar[c])
					{
					xhtml = xhtml + (Html2Xhtml.isEmptyAttr[attrname] ? '"' + attrname + '"' : '""');
			...
			case(7):
				if(Html2Xhtml.isSpaceChar[c])
					{
					xhtml = xhtml + '"' + Html2Xhtml.escapeQuot(attrval, '"') + '"';
			...
			case(12):
				if(c == quot)
					...
					xhtml = xhtml + quot + Html2Xhtml.escapeQuot(phpval, quot) + quot;
